...
export class Versioned implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::versioned::Versioned`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Versioned.$typeName; readonly $fullTypeName: `0x2::versioned::Versioned`; readonly $typeArgs: []; readonly $isPhantom = Versioned.$isPhantom;
 ...
 static reified( ): VersionedReified { const reifiedBcs = Versioned.bcs; return { typeName: Versioned.$typeName, fullTypeName: composeSuiType( Versioned.$typeName, ...[] ) as `0x2::versioned::Versioned`, typeArgs: [ ] as [], isPhantom: Versioned.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Versioned.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Versioned.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Versioned.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Versioned.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Versioned.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Versioned.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Versioned.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Versioned.fetch( client, id, ), new: ( fields: VersionedFields, ) => { return new Versioned( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("Versioned", {
 ...
 static get bcs(): ReturnType<typeof Versioned.instantiateBcs> { if (!Versioned.cachedBcs) { Versioned.cachedBcs = Versioned.instantiateBcs() } return Versioned.cachedBcs };
...
export class VersionChangeCap implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::versioned::VersionChangeCap`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = VersionChangeCap.$typeName; readonly $fullTypeName: `0x2::versioned::VersionChangeCap`; readonly $typeArgs: []; readonly $isPhantom = VersionChangeCap.$isPhantom;
 ...
 private static instantiateBcs() { return bcs.struct("VersionChangeCap", {
 ...
 static get bcs(): ReturnType<typeof VersionChangeCap.instantiateBcs> { if (!VersionChangeCap.cachedBcs) { VersionChangeCap.cachedBcs = VersionChangeCap.instantiateBcs() } return VersionChangeCap.cachedBcs };
