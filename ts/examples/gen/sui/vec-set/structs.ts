...
export class VecSet<K extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::vec_set::VecSet`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = VecSet.$typeName; readonly $fullTypeName: `0x2::vec_set::VecSet<${ToTypeStr<K>}>`; readonly $typeArgs: [ToTypeStr<K>]; readonly $isPhantom = VecSet.$isPhantom;
 ...
 static reified<K extends Reified<TypeArgument, any>>( K: K ): VecSetReified<ToTypeArgument<K>> { const reifiedBcs = VecSet.bcs(toBcs(K)); return { typeName: VecSet.$typeName, fullTypeName: composeSuiType( VecSet.$typeName, ...[extractType(K)] ) as `0x2::vec_set::VecSet<${ToTypeStr<ToTypeArgument<K>>}>`, typeArgs: [ extractType(K) ] as [ToTypeStr<ToTypeArgument<K>>], isPhantom: VecSet.$isPhantom, reifiedTypeArgs: [K], fromFields: (fields: Record<string, any>) => VecSet.fromFields( K, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VecSet.fromFieldsWithTypes( K, item, ), fromBcs: (data: Uint8Array) => VecSet.fromFields( K, reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => VecSet.fromJSONField( K, field, ), fromJSON: (json: Record<string, any>) => VecSet.fromJSON( K, json, ), fromSuiParsedData: (content: SuiParsedData) => VecSet.fromSuiParsedData( K, content, ), fromSuiObjectData: (content: SuiObjectData) => VecSet.fromSuiObjectData( K, content, ), fetch: async (client: SuiClient, id: string) => VecSet.fetch( client, K, id, ), new: ( fields: VecSetFields<ToTypeArgument<K>>, ) => { return new VecSet( [extractType(K)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return <K extends BcsType<any>>(K: K) => bcs.struct(`VecSet<${K.name}>`, {
 ...
 static get bcs(): ReturnType<typeof VecSet.instantiateBcs> { if (!VecSet.cachedBcs) { VecSet.cachedBcs = VecSet.instantiateBcs() } return VecSet.cachedBcs };
