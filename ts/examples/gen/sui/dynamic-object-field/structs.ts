...
export class Wrapper<Name extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::dynamic_object_field::Wrapper`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = Wrapper.$typeName; readonly $fullTypeName: `0x2::dynamic_object_field::Wrapper<${ToTypeStr<Name>}>`; readonly $typeArgs: [ToTypeStr<Name>]; readonly $isPhantom = Wrapper.$isPhantom;
 ...
 static reified<Name extends Reified<TypeArgument, any>>( Name: Name ): WrapperReified<ToTypeArgument<Name>> { const reifiedBcs = Wrapper.bcs(toBcs(Name)); return { typeName: Wrapper.$typeName, fullTypeName: composeSuiType( Wrapper.$typeName, ...[extractType(Name)] ) as `0x2::dynamic_object_field::Wrapper<${ToTypeStr<ToTypeArgument<Name>>}>`, typeArgs: [ extractType(Name) ] as [ToTypeStr<ToTypeArgument<Name>>], isPhantom: Wrapper.$isPhantom, reifiedTypeArgs: [Name], fromFields: (fields: Record<string, any>) => Wrapper.fromFields( Name, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Wrapper.fromFieldsWithTypes( Name, item, ), fromBcs: (data: Uint8Array) => Wrapper.fromFields( Name, reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Wrapper.fromJSONField( Name, field, ), fromJSON: (json: Record<string, any>) => Wrapper.fromJSON( Name, json, ), fromSuiParsedData: (content: SuiParsedData) => Wrapper.fromSuiParsedData( Name, content, ), fromSuiObjectData: (content: SuiObjectData) => Wrapper.fromSuiObjectData( Name, content, ), fetch: async (client: SuiClient, id: string) => Wrapper.fetch( client, Name, id, ), new: ( fields: WrapperFields<ToTypeArgument<Name>>, ) => { return new Wrapper( [extractType(Name)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return <Name extends BcsType<any>>(Name: Name) => bcs.struct(`Wrapper<${Name.name}>`, {
 ...
 static get bcs(): ReturnType<typeof Wrapper.instantiateBcs> { if (!Wrapper.cachedBcs) { Wrapper.cachedBcs = Wrapper.instantiateBcs() } return Wrapper.cachedBcs };
