...
export class Element<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::group_ops::Element`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Element.$typeName; readonly $fullTypeName: `0x2::group_ops::Element<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Element.$isPhantom;
 ...
 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): ElementReified<ToPhantomTypeArgument<T>> { const reifiedBcs = Element.bcs; return { typeName: Element.$typeName, fullTypeName: composeSuiType( Element.$typeName, ...[extractType(T)] ) as `0x2::group_ops::Element<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: Element.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Element.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Element.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Element.fromFields( T, reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Element.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Element.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Element.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Element.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Element.fetch( client, T, id, ), new: ( fields: ElementFields<ToPhantomTypeArgument<T>>, ) => { return new Element( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("Element", {
 ...
 static get bcs(): ReturnType<typeof Element.instantiateBcs> { if (!Element.cachedBcs) { Element.cachedBcs = Element.instantiateBcs() } return Element.cachedBcs };
