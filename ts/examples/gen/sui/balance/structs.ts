...
export class Supply<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::balance::Supply`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Supply.$typeName; readonly $fullTypeName: `0x2::balance::Supply<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Supply.$isPhantom;
 ...
 private static instantiateBcs() { return bcs.struct("Supply", {
 ...
 static get bcs(): ReturnType<typeof Supply.instantiateBcs> { if (!Supply.cachedBcs) { Supply.cachedBcs = Supply.instantiateBcs() } return Supply.cachedBcs };
...
export class Balance<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::balance::Balance`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Balance.$typeName; readonly $fullTypeName: `0x2::balance::Balance<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Balance.$isPhantom;
 ...
 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): BalanceReified<ToPhantomTypeArgument<T>> { const reifiedBcs = Balance.bcs; return { typeName: Balance.$typeName, fullTypeName: composeSuiType( Balance.$typeName, ...[extractType(T)] ) as `0x2::balance::Balance<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: Balance.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Balance.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Balance.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Balance.fromFields( T, reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Balance.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Balance.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Balance.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Balance.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Balance.fetch( client, T, id, ), new: ( fields: BalanceFields<ToPhantomTypeArgument<T>>, ) => { return new Balance( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("Balance", {
 ...
 static get bcs(): ReturnType<typeof Balance.instantiateBcs> { if (!Balance.cachedBcs) { Balance.cachedBcs = Balance.instantiateBcs() } return Balance.cachedBcs };
