...
export class BCS implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::bcs::BCS`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = BCS.$typeName; readonly $fullTypeName: `0x2::bcs::BCS`; readonly $typeArgs: []; readonly $isPhantom = BCS.$isPhantom;
 ...
 static reified( ): BCSReified { const reifiedBcs = BCS.bcs; return { typeName: BCS.$typeName, fullTypeName: composeSuiType( BCS.$typeName, ...[] ) as `0x2::bcs::BCS`, typeArgs: [ ] as [], isPhantom: BCS.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => BCS.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => BCS.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => BCS.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => BCS.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => BCS.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => BCS.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => BCS.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => BCS.fetch( client, id, ), new: ( fields: BCSFields, ) => { return new BCS( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("BCS", {
 ...
 static get bcs(): ReturnType<typeof BCS.instantiateBcs> { if (!BCS.cachedBcs) { BCS.cachedBcs = BCS.instantiateBcs() } return BCS.cachedBcs };
