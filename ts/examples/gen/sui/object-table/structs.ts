...
export class ObjectTable<K extends PhantomTypeArgument, V extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::object_table::ObjectTable`; static readonly $numTypeParams = 2; static readonly $isPhantom = [true,true,] as const;

 readonly $typeName = ObjectTable.$typeName; readonly $fullTypeName: `0x2::object_table::ObjectTable<${PhantomToTypeStr<K>}, ${PhantomToTypeStr<V>}>`; readonly $typeArgs: [PhantomToTypeStr<K>, PhantomToTypeStr<V>]; readonly $isPhantom = ObjectTable.$isPhantom;
 ...
 static reified<K extends PhantomReified<PhantomTypeArgument>, V extends PhantomReified<PhantomTypeArgument>>( K: K, V: V ): ObjectTableReified<ToPhantomTypeArgument<K>, ToPhantomTypeArgument<V>> { const reifiedBcs = ObjectTable.bcs; return { typeName: ObjectTable.$typeName, fullTypeName: composeSuiType( ObjectTable.$typeName, ...[extractType(K), extractType(V)] ) as `0x2::object_table::ObjectTable<${PhantomToTypeStr<ToPhantomTypeArgument<K>>}, ${PhantomToTypeStr<ToPhantomTypeArgument<V>>}>`, typeArgs: [ extractType(K), extractType(V) ] as [PhantomToTypeStr<ToPhantomTypeArgument<K>>, PhantomToTypeStr<ToPhantomTypeArgument<V>>], isPhantom: ObjectTable.$isPhantom, reifiedTypeArgs: [K, V], fromFields: (fields: Record<string, any>) => ObjectTable.fromFields( [K, V], fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ObjectTable.fromFieldsWithTypes( [K, V], item, ), fromBcs: (data: Uint8Array) => ObjectTable.fromFields( [K, V], reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => ObjectTable.fromJSONField( [K, V], field, ), fromJSON: (json: Record<string, any>) => ObjectTable.fromJSON( [K, V], json, ), fromSuiParsedData: (content: SuiParsedData) => ObjectTable.fromSuiParsedData( [K, V], content, ), fromSuiObjectData: (content: SuiObjectData) => ObjectTable.fromSuiObjectData( [K, V], content, ), fetch: async (client: SuiClient, id: string) => ObjectTable.fetch( client, [K, V], id, ), new: ( fields: ObjectTableFields<ToPhantomTypeArgument<K>, ToPhantomTypeArgument<V>>, ) => { return new ObjectTable( [extractType(K), extractType(V)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("ObjectTable", {
 ...
 static get bcs(): ReturnType<typeof ObjectTable.instantiateBcs> { if (!ObjectTable.cachedBcs) { ObjectTable.cachedBcs = ObjectTable.instantiateBcs() } return ObjectTable.cachedBcs };
