...
export class ObjectBag implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::object_bag::ObjectBag`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = ObjectBag.$typeName; readonly $fullTypeName: `0x2::object_bag::ObjectBag`; readonly $typeArgs: []; readonly $isPhantom = ObjectBag.$isPhantom;
 ...
 static reified( ): ObjectBagReified { const reifiedBcs = ObjectBag.bcs; return { typeName: ObjectBag.$typeName, fullTypeName: composeSuiType( ObjectBag.$typeName, ...[] ) as `0x2::object_bag::ObjectBag`, typeArgs: [ ] as [], isPhantom: ObjectBag.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => ObjectBag.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => ObjectBag.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => ObjectBag.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => ObjectBag.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => ObjectBag.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => ObjectBag.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => ObjectBag.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => ObjectBag.fetch( client, id, ), new: ( fields: ObjectBagFields, ) => { return new ObjectBag( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("ObjectBag", {
 ...
 static get bcs(): ReturnType<typeof ObjectBag.instantiateBcs> { if (!ObjectBag.cachedBcs) { ObjectBag.cachedBcs = ObjectBag.instantiateBcs() } return ObjectBag.cachedBcs };
