...
export class Url implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::url::Url`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Url.$typeName; readonly $fullTypeName: `0x2::url::Url`; readonly $typeArgs: []; readonly $isPhantom = Url.$isPhantom;
 ...
 static reified( ): UrlReified { const reifiedBcs = Url.bcs; return { typeName: Url.$typeName, fullTypeName: composeSuiType( Url.$typeName, ...[] ) as `0x2::url::Url`, typeArgs: [ ] as [], isPhantom: Url.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Url.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Url.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Url.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Url.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Url.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Url.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Url.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Url.fetch( client, id, ), new: ( fields: UrlFields, ) => { return new Url( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("Url", {
 ...
 static get bcs(): ReturnType<typeof Url.instantiateBcs> { if (!Url.cachedBcs) { Url.cachedBcs = Url.instantiateBcs() } return Url.cachedBcs };
