...
export class VerifiedID implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::zklogin_verified_id::VerifiedID`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = VerifiedID.$typeName; readonly $fullTypeName: `0x2::zklogin_verified_id::VerifiedID`; readonly $typeArgs: []; readonly $isPhantom = VerifiedID.$isPhantom;
 ...
 static reified( ): VerifiedIDReified { const reifiedBcs = VerifiedID.bcs; return { typeName: VerifiedID.$typeName, fullTypeName: composeSuiType( VerifiedID.$typeName, ...[] ) as `0x2::zklogin_verified_id::VerifiedID`, typeArgs: [ ] as [], isPhantom: VerifiedID.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => VerifiedID.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => VerifiedID.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => VerifiedID.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => VerifiedID.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => VerifiedID.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => VerifiedID.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => VerifiedID.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => VerifiedID.fetch( client, id, ), new: ( fields: VerifiedIDFields, ) => { return new VerifiedID( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("VerifiedID", {
 ...
 static get bcs(): ReturnType<typeof VerifiedID.instantiateBcs> { if (!VerifiedID.cachedBcs) { VerifiedID.cachedBcs = VerifiedID.instantiateBcs() } return VerifiedID.cachedBcs };
