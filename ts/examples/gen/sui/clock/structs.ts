...
export class Clock implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::clock::Clock`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Clock.$typeName; readonly $fullTypeName: `0x2::clock::Clock`; readonly $typeArgs: []; readonly $isPhantom = Clock.$isPhantom;
 ...
 static reified( ): ClockReified { const reifiedBcs = Clock.bcs; return { typeName: Clock.$typeName, fullTypeName: composeSuiType( Clock.$typeName, ...[] ) as `0x2::clock::Clock`, typeArgs: [ ] as [], isPhantom: Clock.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Clock.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Clock.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Clock.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Clock.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Clock.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Clock.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Clock.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Clock.fetch( client, id, ), new: ( fields: ClockFields, ) => { return new Clock( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("Clock", {
 ...
 static get bcs(): ReturnType<typeof Clock.instantiateBcs> { if (!Clock.cachedBcs) { Clock.cachedBcs = Clock.instantiateBcs() } return Clock.cachedBcs };
