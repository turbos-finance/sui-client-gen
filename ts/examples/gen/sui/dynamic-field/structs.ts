...
export class Field<Name extends TypeArgument, Value extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::dynamic_field::Field`; static readonly $numTypeParams = 2; static readonly $isPhantom = [false,false,] as const;

 readonly $typeName = Field.$typeName; readonly $fullTypeName: `0x2::dynamic_field::Field<${ToTypeStr<Name>}, ${ToTypeStr<Value>}>`; readonly $typeArgs: [ToTypeStr<Name>, ToTypeStr<Value>]; readonly $isPhantom = Field.$isPhantom;
 ...
 private static instantiateBcs() { return <Name extends BcsType<any>, Value extends BcsType<any>>(Name: Name, Value: Value) => bcs.struct(`Field<${Name.name}, ${Value.name}>`, {
 ...
 static get bcs(): ReturnType<typeof Field.instantiateBcs> { if (!Field.cachedBcs) { Field.cachedBcs = Field.instantiateBcs() } return Field.cachedBcs };
