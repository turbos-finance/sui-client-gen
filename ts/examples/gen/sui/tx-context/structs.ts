...
export class TxContext implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::tx_context::TxContext`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = TxContext.$typeName; readonly $fullTypeName: `0x2::tx_context::TxContext`; readonly $typeArgs: []; readonly $isPhantom = TxContext.$isPhantom;
 ...
 static reified( ): TxContextReified { const reifiedBcs = TxContext.bcs; return { typeName: TxContext.$typeName, fullTypeName: composeSuiType( TxContext.$typeName, ...[] ) as `0x2::tx_context::TxContext`, typeArgs: [ ] as [], isPhantom: TxContext.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => TxContext.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => TxContext.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => TxContext.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => TxContext.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => TxContext.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => TxContext.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => TxContext.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => TxContext.fetch( client, id, ), new: ( fields: TxContextFields, ) => { return new TxContext( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("TxContext", {
 ...
 static get bcs(): ReturnType<typeof TxContext.instantiateBcs> { if (!TxContext.cachedBcs) { TxContext.cachedBcs = TxContext.instantiateBcs() } return TxContext.cachedBcs };
