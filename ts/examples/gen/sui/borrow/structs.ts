...
export class Referent<T extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::borrow::Referent`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = Referent.$typeName; readonly $fullTypeName: `0x2::borrow::Referent<${ToTypeStr<T>}>`; readonly $typeArgs: [ToTypeStr<T>]; readonly $isPhantom = Referent.$isPhantom;
 ...
 static reified<T extends Reified<TypeArgument, any>>( T: T ): ReferentReified<ToTypeArgument<T>> { const reifiedBcs = Referent.bcs(toBcs(T)); return { typeName: Referent.$typeName, fullTypeName: composeSuiType( Referent.$typeName, ...[extractType(T)] ) as `0x2::borrow::Referent<${ToTypeStr<ToTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [ToTypeStr<ToTypeArgument<T>>], isPhantom: Referent.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Referent.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Referent.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Referent.fromFields( T, reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Referent.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Referent.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Referent.fromSuiParsedData( T, content, ), fromSuiObjectData: (content: SuiObjectData) => Referent.fromSuiObjectData( T, content, ), fetch: async (client: SuiClient, id: string) => Referent.fetch( client, T, id, ), new: ( fields: ReferentFields<ToTypeArgument<T>>, ) => { return new Referent( [extractType(T)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return <T extends BcsType<any>>(T: T) => bcs.struct(`Referent<${T.name}>`, {
 ...
 static get bcs(): ReturnType<typeof Referent.instantiateBcs> { if (!Referent.cachedBcs) { Referent.cachedBcs = Referent.instantiateBcs() } return Referent.cachedBcs };
...
export class Borrow implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x2::borrow::Borrow`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Borrow.$typeName; readonly $fullTypeName: `0x2::borrow::Borrow`; readonly $typeArgs: []; readonly $isPhantom = Borrow.$isPhantom;
