...
export class FixedPoint32 implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x1::fixed_point32::FixedPoint32`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = FixedPoint32.$typeName; readonly $fullTypeName: `0x1::fixed_point32::FixedPoint32`; readonly $typeArgs: []; readonly $isPhantom = FixedPoint32.$isPhantom;
 ...
 static reified( ): FixedPoint32Reified { const reifiedBcs = FixedPoint32.bcs; return { typeName: FixedPoint32.$typeName, fullTypeName: composeSuiType( FixedPoint32.$typeName, ...[] ) as `0x1::fixed_point32::FixedPoint32`, typeArgs: [ ] as [], isPhantom: FixedPoint32.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => FixedPoint32.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => FixedPoint32.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => FixedPoint32.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => FixedPoint32.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => FixedPoint32.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => FixedPoint32.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => FixedPoint32.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => FixedPoint32.fetch( client, id, ), new: ( fields: FixedPoint32Fields, ) => { return new FixedPoint32( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("FixedPoint32", {
 ...
 static get bcs(): ReturnType<typeof FixedPoint32.instantiateBcs> { if (!FixedPoint32.cachedBcs) { FixedPoint32.cachedBcs = FixedPoint32.instantiateBcs() } return FixedPoint32.cachedBcs };
