...
export class Option<Element extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x1::option::Option`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 __inner: Element = null as unknown as Element; // for type checking in reified.ts

; readonly $typeName = Option.$typeName; readonly $fullTypeName: `0x1::option::Option<${ToTypeStr<Element>}>`; readonly $typeArgs: [ToTypeStr<Element>]; readonly $isPhantom = Option.$isPhantom;
 ...
 static reified<Element extends Reified<TypeArgument, any>>( Element: Element ): OptionReified<ToTypeArgument<Element>> { const reifiedBcs = Option.bcs(toBcs(Element)); return { typeName: Option.$typeName, fullTypeName: composeSuiType( Option.$typeName, ...[extractType(Element)] ) as `0x1::option::Option<${ToTypeStr<ToTypeArgument<Element>>}>`, typeArgs: [ extractType(Element) ] as [ToTypeStr<ToTypeArgument<Element>>], isPhantom: Option.$isPhantom, reifiedTypeArgs: [Element], fromFields: (fields: Record<string, any>) => Option.fromFields( Element, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Option.fromFieldsWithTypes( Element, item, ), fromBcs: (data: Uint8Array) => Option.fromFields( Element, reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Option.fromJSONField( Element, field, ), fromJSON: (json: Record<string, any>) => Option.fromJSON( Element, json, ), fromSuiParsedData: (content: SuiParsedData) => Option.fromSuiParsedData( Element, content, ), fromSuiObjectData: (content: SuiObjectData) => Option.fromSuiObjectData( Element, content, ), fetch: async (client: SuiClient, id: string) => Option.fetch( client, Element, id, ), new: ( fields: OptionFields<ToTypeArgument<Element>>, ) => { return new Option( [extractType(Element)], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return <Element extends BcsType<any>>(Element: Element) => bcs.struct(`Option<${Element.name}>`, {
 ...
 static get bcs(): ReturnType<typeof Option.instantiateBcs> { if (!Option.cachedBcs) { Option.cachedBcs = Option.instantiateBcs() } return Option.cachedBcs };
