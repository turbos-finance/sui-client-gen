...
export class TypeName implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `0x1::type_name::TypeName`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = TypeName.$typeName; readonly $fullTypeName: `0x1::type_name::TypeName`; readonly $typeArgs: []; readonly $isPhantom = TypeName.$isPhantom;
 ...
 static reified( ): TypeNameReified { const reifiedBcs = TypeName.bcs; return { typeName: TypeName.$typeName, fullTypeName: composeSuiType( TypeName.$typeName, ...[] ) as `0x1::type_name::TypeName`, typeArgs: [ ] as [], isPhantom: TypeName.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => TypeName.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => TypeName.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => TypeName.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => TypeName.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => TypeName.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => TypeName.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => TypeName.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => TypeName.fetch( client, id, ), new: ( fields: TypeNameFields, ) => { return new TypeName( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("TypeName", {
 ...
 static get bcs(): ReturnType<typeof TypeName.instantiateBcs> { if (!TypeName.cachedBcs) { TypeName.cachedBcs = TypeName.instantiateBcs() } return TypeName.cachedBcs };
