...
export class EXAMPLE_COIN implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::example_coin::EXAMPLE_COIN`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = EXAMPLE_COIN.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::example_coin::EXAMPLE_COIN`; readonly $typeArgs: []; readonly $isPhantom = EXAMPLE_COIN.$isPhantom;
 ...
 static reified( ): EXAMPLE_COINReified { const reifiedBcs = EXAMPLE_COIN.bcs; return { typeName: EXAMPLE_COIN.$typeName, fullTypeName: composeSuiType( EXAMPLE_COIN.$typeName, ...[] ) as `${typeof PKG_V1}::example_coin::EXAMPLE_COIN`, typeArgs: [ ] as [], isPhantom: EXAMPLE_COIN.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => EXAMPLE_COIN.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => EXAMPLE_COIN.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => EXAMPLE_COIN.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => EXAMPLE_COIN.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => EXAMPLE_COIN.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => EXAMPLE_COIN.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => EXAMPLE_COIN.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => EXAMPLE_COIN.fetch( client, id, ), new: ( fields: EXAMPLE_COINFields, ) => { return new EXAMPLE_COIN( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("EXAMPLE_COIN", {
 ...
 static get bcs(): ReturnType<typeof EXAMPLE_COIN.instantiateBcs> { if (!EXAMPLE_COIN.cachedBcs) { EXAMPLE_COIN.cachedBcs = EXAMPLE_COIN.instantiateBcs() } return EXAMPLE_COIN.cachedBcs };
 ...
 static fromSuiObjectData( data: SuiObjectData ): EXAMPLE_COIN { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isEXAMPLE_COIN(data.bcs.type)) { throw new Error(`object at is not a EXAMPLE_COIN object`); }

 return EXAMPLE_COIN.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return EXAMPLE_COIN.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export class Faucet implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::example_coin::Faucet`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Faucet.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::example_coin::Faucet`; readonly $typeArgs: []; readonly $isPhantom = Faucet.$isPhantom;
 ...
 static reified( ): FaucetReified { const reifiedBcs = Faucet.bcs; return { typeName: Faucet.$typeName, fullTypeName: composeSuiType( Faucet.$typeName, ...[] ) as `${typeof PKG_V1}::example_coin::Faucet`, typeArgs: [ ] as [], isPhantom: Faucet.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Faucet.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Faucet.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Faucet.fromFields( reifiedBcs.parse(data) ), bcs: reifiedBcs, fromJSONField: (field: any) => Faucet.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Faucet.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Faucet.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Faucet.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Faucet.fetch( client, id, ), new: ( fields: FaucetFields, ) => { return new Faucet( [], fields ) }, kind: "StructClassReified", } }
 ...
 private static instantiateBcs() { return bcs.struct("Faucet", {
 ...
 static get bcs(): ReturnType<typeof Faucet.instantiateBcs> { if (!Faucet.cachedBcs) { Faucet.cachedBcs = Faucet.instantiateBcs() } return Faucet.cachedBcs };
 ...
 static fromSuiObjectData( data: SuiObjectData ): Faucet { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isFaucet(data.bcs.type)) { throw new Error(`object at is not a Faucet object`); }

 return Faucet.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Faucet.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
